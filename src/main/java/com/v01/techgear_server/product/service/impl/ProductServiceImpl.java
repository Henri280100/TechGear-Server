...
import com.v01.techgear_server.shared.event.producer.product.ProductEventProducer;
...
//    private final DiscountService discountService;
	...
	@Async
	protected CompletableFuture<String> saveImage(MultipartFile image) throws IOException {
		return fileStorageService.storeSingleImage(image)
				.thenApply(ImageDTO::getImageUrl)
				.exceptionally(ex -> {
					log.error("Error storing image", ex);
					throw new BadRequestException("Failed to store image", ex);
				});
	}
	...
	@Transactional(
			...
			noRollbackFor = {ValidationException.class}, isolation = Isolation.READ_COMMITTED,
		...
		try {
			...
			TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
				@Override public void afterCommit() {
					...
					event.setEventType(ProductEventType.PRODUCT_CREATED);
					event.setPayload(Map.of("products", savedProducts.stream().map(p -> Map.of(
							"productId", p.getProductId(),
							"productName", p.getName(),
							"productDescription", p.getProductDescription(),
							"productImage", p.getImageUrl(),
							"productCategory", p.getCategory().getCategoryName()
					)).collect(Collectors.toList())));
					productEventProducer.publishProductEvent(event);
				}
			});
	...
	@Transactional(
			...
			noRollbackFor = {ValidationException.class}, isolation = Isolation.READ_COMMITTED,
			propagation = Propagation.REQUIRED, timeout = 30)
	public CompletableFuture<List<ProductDetailDTO>> createProductDetail(List<ProductDetailDTO> productDetailDTOs,
		...
		try {
			...
			long daysSinceRelease = ChronoUnit.DAYS.between(LocalDate.now(),
					productDetails.getFirst().getReleaseDate().toLocalDate());
			 ...
			 * </ul>
			 */
			ProductStatus status = switch ((int) ChronoUnit.DAYS.between(
					productDetails.getFirst().getReleaseDate().toLocalDate(), LocalDate.now())) {
			...
			productDetails.forEach(pd -> {
				...
				List<ProductSpecificationDTO> filteredDTOs = Optional.ofNullable(
								productDetailDTOs.getFirst().getSpecification())
						.orElse(Collections.emptyList())
						.stream()
				...
				List<ProductSpecification> specEntities = filteredDTOs.stream()
						.map(dto -> {
							ProductSpecification specEntity = productSpecificationMapper.toEntity(
									dto); // Map DTO to Entity
			...
			payload.put("productDetails", savedProductDetails.stream().map(pd -> Map.of(
					"productId", pd.getProduct().getProductId(),
					"productDetailId", pd.getId(),
					"productName", pd.getProduct().getName(),
					"productDescription", pd.getProductDetailsDesc(),
					"productImage", pd.getDetailImageUrl(),
					"productVideo", pd.getDetailVideoUrl(),
					"productStatus", pd.getProductStatus(),
					"finalPrice", pd.getFinalPrice(),
					"hype", pd.getHype()
			)).collect(Collectors.toList()));
			
			ProductEvent event = new ProductEvent();
			event.setEventType(ProductEventType.PRODUCT_UPDATED);
			event.setPayload(payload);
			event.setProductId(savedProductDetails.getFirst().getProduct().getProductId());
			
			for (ProductDetail pd : savedProductDetails) {
				Long productId = pd.getProduct().getProductId();
				
				productEventProducer.publishProductEvent(
						new ProductEvent(ProductEventType.IMAGE_UPDATED, productId,
								Map.of("imageUrl", pd.getDetailImageUrl()))
				);
				
				productEventProducer.publishProductEvent(
						new ProductEvent(ProductEventType.FINAL_PRICE_CHANGED, productId,
								Map.of("finalPrice", pd.getFinalPrice()))
				);
			}
	...
	@Override
	@Async
	@CachePut(value = "productCache", key = "#productId", unless = "#result == null")
	...
	public CompletableFuture<ProductDTO> updateProduct(Long productId, ProductDTO productDTO, MultipartFile image) {
		return CompletableFuture.supplyAsync(() -> {
			...
			if (image != null && !image.isEmpty()) {
				try {
					savedImage = String.valueOf(saveImage(image));
				} catch (IOException e) {
	...
	@Caching(
			...
			evict = @CacheEvict(value = "productSearchCache", allEntries = true, beforeInvocation = true)
	)
	@Transactional(rollbackFor = {ResourceNotFoundException.class, DataAccessException.class})
	public CompletableFuture<ProductDTO> updateProductImage(Long productId, MultipartFile image) {
		try {
			if (image == null || image.isEmpty()) return null;
			
			log.info("Uploading image for product ID: {}", productId);
			String savedImageUrl = saveImage(image).join(); // Cloudinary
			...
			// Refresh product in Typesense
			ProductDTO productDTO = productMapper.toDTO(product);
			productIndexingService.indexSingleProduct(productDTO); // <- new step
			
			return CompletableFuture.completedFuture(productDTO);
		} catch (Exception ex) {
			log.error("Failed to update product image", ex);
	...
	@Override
	@Cacheable(value = "productCache", keyGenerator = "customKeyGenerator", condition = "#category != null",
			unless = "#result == null")
	@Transactional(readOnly = true)
