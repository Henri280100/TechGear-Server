...
public interface TokenRepository extends JpaRepository<Token, Long> {
    ...
    @Query("SELECT t FROM Token t WHERE t.userId = :userId AND t.revoked = false AND t.expiresAt > CURRENT_TIMESTAMP")
    List<Token> findAllValidTokensByUser(@Param("userId") Long userId);

    @Query("SELECT t FROM Token t WHERE t.accessToken = :accessToken AND t.revoked = false ORDER BY t.createdAt DESC")
    Optional<Token> findLatestValidAccessToken(@Param("accessToken") String accessToken);

    @Query("SELECT t FROM Token t WHERE t.userId = :userId AND t.revoked = false ORDER BY t.createdAt DESC")
